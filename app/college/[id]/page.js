...
import React, { useEffect, useState } from 'react';
import { useParams } from 'next/navigation';
// Saare icons jo aapne use kiye hain
import {
   MapPin, Building2, CheckCircle, Send, Plus, Globe, Phone, Mail,
   Calendar, TrendingUp, ArrowRight, Heart, MessageCircle, Share2,
   ExternalLink, Trash2
} from 'lucide-react';
// API Import (Isko apne sahi path se change karein)
import { getCollegeById } from "@/app/utils/collegeApi"; 
import { GlobalLoader } from '@/app/components/Loader';

export default function CollegeProfilePage() {
   const params = useParams();
   const [activeTab, setActiveTab] = useState('Overview');
   const [data, setData] = useState(null); // Dynamic data state
   const [loading, setLoading] = useState(true);
   const [isFollowing, setIsFollowing] = useState(false);

   // Feeds state (Static for now as per your code)
   const [feeds, setFeeds] = useState([
      {
         id: 1,
         content: "We're excited to announce our new AI & Machine Learning Center of Excellence!",
         timestamp: "2 hours ago",
         likes: 45,
         comments: 12,
         shares: 8,
   ...
   useEffect(() => {
      const fetchCollegeData = async () => {
         ...
         setLoading(true);
         const result = await getCollegeById(params.id);
         
         if (result.success) {
            setData(result.data); // Backend se aaya data yahan set hoga
         } else {
   ...
   const handleLike = (postId) => {
      setFeeds(feeds.map(post => post.id === postId ? { ...post, liked: !post.liked, likes: post.liked ? post.likes - 1 : post.likes + 1 } : post));
   };
   ...
   const renderOverviewTab = () => (
      <div className="space-y-4">
         <div className="bg-white rounded-xl border border-gray-300 p-6 ">
            <h2 className="text-lg font-bold text-gray-900 mb-4">About</h2>
            <p className="text-sm text-gray-700 leading-relaxed">
               {data.description || data.about || "No description available for this institute."}
            </p>
         </div>
         ...
         <div className="bg-white rounded-xl border border-gray-300 p-6 ">
            <div className="flex items-center justify-between mb-4">
               <h2 className="text-lg font-bold text-gray-900">Recent Posts</h2>
               <button onClick={() => setActiveTab('Posts')} className="text-sm text-blue-600 font-semibold flex items-center gap-1">
                  View all <ArrowRight size={16} />
               </button>
            </div>
            {feeds.slice(0, 1).map(post => (
               <div key={post.id} className="p-4 border border-gray-200 rounded-lg">
                  <h3 className="font-semibold text-gray-900 text-sm">{post.content.substring(0, 100)}...</h3>
               </div>
            ))}
         </div>
      </div>
      ...
      <div className="min-h-screen bg-[#fafafa] font-sans">
         <main className="max-w-7xl mx-auto px-4 py-6 grid grid-cols-1 lg:grid-cols-4 gap-6 mt-15">
            <div className="lg:col-span-3 space-y-4">
               {/* HERO CARD DYNAMIC */}
               <div className="bg-white rounded-xl border border-[#dbdbdb] overflow-hidden">
                  <div className="h-48 bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-[#dbdbdb] relative">
                     {/* Cover image can go here */}
                  </div>

                  <div className="px-6 pb-6 pt-0 relative">
                     <div className="-mt-16 mb-4 inline-block">
                        <div className="w-32 h-32 bg-white rounded-lg border border-[#dbdbdb] flex items-center justify-center overflow-hidden shadow-sm">
                           {data.logo ? (
                     ...
                     <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                        <div>
                           <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                              {data.name}
                              {data.isVerified && <CheckCircle size={20} className="text-blue-600 fill-blue-600" />}
                           </h1>
                           <p className="text-gray-900 mt-1 font-medium">{data.tagline || data.industry}</p>

                           <div className="flex flex-wrap items-center gap-4 mt-2 text-sm text-gray-500">
                              <span className="flex items-center gap-1"><Building2 size={14} /> {data.orgType || "University"}</span>
                              <span className="w-1 h-1 bg-gray-400 rounded-full"></span>
                              <span className="flex items-center gap-1"><MapPin size={14} /> {data.city || data.location}</span>
                              {data.established && (
                                 <>
                                    <span className="w-1 h-1 bg-gray-400 rounded-full"></span>
                                    <span className="text-blue-600 font-semibold">Est. {data.established}</span>
                                 </>
                              )}
                           </div>

                           <div className="flex items-center gap-2 mt-4">
                              <button onClick={handleFollow} className={`px-6 py-2 rounded-full font-semibold transition flex items-center gap-1 ${isFollowing ? 'bg-gray-200 text-gray-700' : 'bg-[#0013E3] text-white hover:bg-blue-800'}`}>
                                 {isFollowing ? 'Following' : 'Follow'}
                              </button>
                              <button className="flex gap-1 border border-blue-600 text-blue-600 px-4 py-2 rounded-full text-sm font-medium hover:bg-blue-50">
                        ...
                        <div className="flex gap-8 overflow-x-auto">
                           {['Overview', 'About', 'Posts', 'Jobs', 'People'].map((tab) => (
                              <button
                                 ...
                                 className={`py-3 text-sm font-semibold border-b-[3px] whitespace-nowrap transition ${activeTab === tab ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-800'}`}
                              >
                                 {tab}
                              </button>
                           ))}
               ...
               {/* TAB CONTENT */}
               {activeTab === 'Overview' && renderOverviewTab()}
               {/* Baaki tabs aapke original render logic se chalenge but data state use karenge */}
            </div>
            ...
            <div className="space-y-4">
               <div className="bg-white rounded-xl border border-gray-300 p-4 ">
                  ...
                  <div className="space-y-3">
                     {data.email && <div className="flex items-center gap-2 text-xs text-gray-600"><Mail size={14}/> {data.email}</div>}
                     {data.website && <div className="flex items-center gap-2 text-xs text-blue-600"><Globe size={14}/> Website</div>}
                  </div>
               </div>
            </div>
         </main>
